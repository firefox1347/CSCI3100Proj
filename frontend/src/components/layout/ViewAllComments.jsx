  ...
  const ViewAllComments = ({ open, onClose, postid }) => {
    ...
    const debugFetchComments = async () => {
      try {
        ...
        toast.error("Fetch Failed: " + (error.response?.data?.message || error.message));
        throw error;
      }
    };
    ...
    const handleSubCommentLike = async (commentid, subcommentid) => {
      try {
        ...
        const response = await axiosInstance.put(`/comments/${commentid}/${subcommentid}/like`);
        console.log("Like response:", response.data);
                ...
                <Stack spacing={0.5}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mt: 0.5 }}>
                      ...
                      <IconButton size="small">
                        <ReplyIcon fontSize="small" onClick={() => handleReplyClick(comment._id, comment.author.username)}/>
                      </IconButton>
                      ...
                      {comment.noOfSubComment > 0 && (
                        ...
                        >
                          View {comment.noOfSubComment} {comment.noOfSubComment === 1 ? 'reply' : 'replies'}
                        </Button>
                      )}
                  </Box>
                  
                  {/* Subcomments */}
                  {showReplies[comment._id] && comment.subComment?.map((sub) => (
                    <Box key={sub._id} sx={{ ml: 3, mt: 1, borderLeft: '2px solid #ddd', pl: 1.5 }}>
                      <Typography variant="body2" sx={{ fontWeight: 'bold' }}>
                      ...
                      <IconButton 
                        size="small" 
                        onClick={() => handleSubCommentLike(comment._id, sub._id)}
                      >
                        ...
                        <Typography variant="caption">{sub.noOfLikes}</Typography>
                      </Box>
                    </Box>
                  ))}

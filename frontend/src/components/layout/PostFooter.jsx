...
const PostFooter = ({ username }) => {
  ...
  const [comments, setComments] = useState([{ user: username, text: "Test Comment" }]); // thiss should be replaced from test comment to the ORIGINAL comment by the user
  ...
  const handlePostComment = () => {
    if (newComment.trim()) {
      setComments([...comments, { user: currentUser, text: newComment.trim() }]);
      setNewComment('');
    }
  };
  ...
  return (
    <Box sx={{ mb: 10 }}>
      ...
      <Typography variant="body2" fontWeight="bold" sx={{ mb: 1, color: 'black' }}>
        {likes.toLocaleString()} likes
      </Typography>

      {comments.slice(0, 2).map((comment, index) => (
        <Typography key={index} variant="body2" sx={{ mb: 0.5, color: 'black' }}>
